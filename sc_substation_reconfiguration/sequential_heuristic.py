"""Seq-H sequential heuristic from the Section IV-A case study.

Seq-H solves each substation topology problem (MP_i) one at a time while
allowing the dispatch to update between iterations. Substation configurations
are optimized in sequence until no further improvement is found.
"""
...
def create_sequential_heuristic_model(
    data,
    cont_list=None,
    Max_Sw_bus=0,
    Up_redispatch=0,
    ...
    """Build the reusable MIP for the Seq-H substation solves.
...
def solve_sequential_substation(
    data,
    TopologyFix,
    model0,
    cont_list,
    ...
    """Optimize one substation while fixing all other topologies.

    Args:
        data: Network and model-parameter dictionary returned by
            ``read_data_AC``.
        TopologyFix: Current topology inherited from earlier Seq-H solves.
        model0: Reusable MIP created by ``create_sequential_heuristic_model``.
        cont_list: Contingency identifiers represented by ``model0``.
        substation: Substation whose topology is free in this solve.
        print_result: Print detailed solution information when ``True``.

    Returns:
        A dictionary containing solve time, optimized topology, dispatch,
        optimality gap, costs, load shedding, and updated voltage and angle
    ...
    # Fix every topology except the selected substation.
    for b in Bus:
        if b!=substation:
            ...
            for l,i,j in Lines.select('*',b,'*'):
                model.addConstr((  model.getVarByName('z_li['+str(l)+','+str(i)+','+str(j)+']') == TopologyFix['l_i'][l,i,j] ), name='eqFix')
                model.getVarByName('z_li['+str(l)+','+str(i)+','+str(j)+']').Start = TopologyFix['l_i'][l,i,j]
            for d,i in D2B.select('*',b):
                model.addConstr((  model.getVarByName('z_d['+str(d)+']') == TopologyFix['d'][d] ), name='eqFix')
                model.getVarByName('z_d['+str(d)+']').Start = TopologyFix['d'][d]
            for g,i in G2B.select('*',b):
                model.addConstr((  model.getVarByName('z_g['+str(g)+']') == TopologyFix['g'][g]), name='eqFix')
                model.getVarByName('z_g['+str(g)+']').Start = TopologyFix['g'][g]


    model.update()
    # Solve the selected substation problem.
    ...
    # Build the optimized topology dictionary.
...
def SC_SR_SeqH(
    ...
    """Run the Seq-H substation-by-substation optimization sequence.

    Seq-H solves each substation topology problem (MP_i) one at a time while
    allowing dispatch to update between iterations. Substation configurations
    are optimized in sequence until no further improvement is found.
    ...
    Returns:
        A dictionary containing per-substation topology and cost histories,
        final dispatch and shedding, and total solution time.
    """
    ...
    # Build one reusable model for all substation solves.
    model0 = create_sequential_heuristic_model(data=data,cont_list=cont_list,
                                        Max_Sw_bus=Max_Sw_bus,
    ...
    # Start from the default unsplit topology.
    ...
    TopologyFix_updated = topology_default0.copy()

    # Sequential substation-optimization loop
    for sub in tqdm(Bus):

        iter = Bus.index(sub)


        res = solve_sequential_substation(data=data,substation=sub,TopologyFix=TopologyFix_updated,model0=model0,cont_list=cont_list,print_result=print_result)


        TopologyFix_updated = res['TopologyDict']
        UB = res['Cost_tot']
        tot_time += res['time']
