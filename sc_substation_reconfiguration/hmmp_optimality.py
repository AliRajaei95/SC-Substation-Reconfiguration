"""HMMP optimality-cut comparison model from the Section IV-A case study.

This variant augments the multi-master decomposition with optimality-oriented
substation cuts for comparison with the other solution approaches.
"""
...
# %% [markdown]
# # This version is designed for optimal busbar splitting
# 
...
# %%
def create_FSP_line_v66(data,Up_redispatch=0,Dn_redispatch=1.0,cont_list=None):
...
#                             for l,i,j in Lines for c in cont_list if l!=c  ) , name='eqQloss')
    ...
    eq_balance_busbar1=model.addConstrs((                                        
                ...
                -quicksum( Pdi[d,m] - Pdi_Shed[d,m,ll]  for d,b in D2B.select('*',b)   ) + sp_up[b,m,ll] - sp_dn[b,m,ll] ==
    ...
    eq_Qbalance_busbar1=model.addConstrs((                                        
                ...
                -quicksum( Qdi[d,m] - Qdi_Shed[d,m,ll]   for d,b in D2B.select('*',b)   ) + sq_up[b,m,ll] - sq_dn[b,m,ll] ==
...
# %%
def solve_FSP_line_v66(data,TopologyMP,model0,
                                    print_result=False):
...
# %%
def create_OSP_substation_v66(data,cont_list=None,
...
# %%
def solve_OSP_substation_v66(data,TopologyMP,model0,cont_list=None):
...
# %%
def create_MP2_sub_v66(data,substation, #the substation to find the configuration
                            Probabilistic=False,
...
#=============================== Variables ==================================================
    ...
    # eq_bg=model.addConstrs((  z_bus[b]-1+z_g[g]<=0     
    #                         for b in Bus for g in G ), name='eq_bg')
    # eq_bd=model.addConstrs((  z_bus[b]-1+z_d[d]<=0     
    #                         for b in Bus for d in DemandSet ), name='eq_bd')
    # eq_blte=model.addConstrs((  z_bus[b]-1 +z_li[l,i,j]    <=0     
    #                         for b in Bus 
    #                      for b in Bus for l,i,j in Lines.select('*',b,'*') 
    #                      ), name='eq_blte')
...
# %%
def solve_MP2_sub_v66(data,substation, #the substation to find the configuration
                           MP2model,OSPmodel,FSPmodels,
                           Zfixed=None,split_list=[], PgFix=None,QgFix=None,
                         line_cont_list=[], Max_FSP_iter=20,FSP_criteria=0):
...
#             break
    ...
    # result = solve_OSP_substation_v66(data=data,cont_list=sub_cont_list,TopologyMP=TopologyMP, model0=OSPmodel)
    # OF_OSP=result['OF_OSP']
    # Mu=result['Mu']
    # tot_time += result['time']

    # print('MP2 RD Shed: ',model.objVal)
    # print('OSP: ',OF_OSP)
...
# %%
def BCC_v66_AC_MultiBenders_OptimalSplit(data,line_cont_list=[],
                            Max_iter=10,
...
#=============================== Variables ==================================================
    ...
    line_cont_list_0 = line_cont_list + [0]
...
#     model.addConstr(OF_MP == GenCost + TotalPhi   ,name='Eq_OF_MP')
    ...
    PdShed_dic = pd.DataFrame(0,columns=['shed'],index=pd.MultiIndex.from_product([DemandSet,busbar,all_cont]), dtype=float)
    ...
    for c in line_cont_notradial:
        FSP_models[c] = create_FSP_line_v66(data=data,cont_list=[c])
    
    for c in line_cont_list: #line_cont_radial:
        OSP_models[c] = create_OSP_substation_v66(data=data,cont_list=[c],Up_redispatch=Up_redispatch,Dn_redispatch=Dn_redispatch,Max_Sw_bus=Max_Sw_bus)

    for sub in Bus:
        MP2_models[sub] = create_MP2_sub_v66(data=data,substation=sub,Max_Sw_bus=Max_Sw_bus,Up_redispatch=Up_redispatch,Dn_redispatch=Dn_redispatch,
                                             FixedCost=FixedCost,Alpha=Alpha,Pg_market=Pg_market,Probabilistic=Probabilistic)
        
        sub_cont_list = [sub,str(sub+'-1'),str(sub+'-2')]
        # sub_cont_list=[]
        OSP_models[sub]=create_OSP_substation_v66(data=data,cont_list=sub_cont_list
    ...
    while k < Max_iter:
        ...
        while k1 < Max_FSP_iter:
            ...
            for sub in (Bus):
                ...
                result=solve_MP2_sub_v66(data=data,substation=sub,
            ...
            for c in (line_cont_notradial):
#                 print('\t\tcont %s'%(c),end='')
                result = solve_FSP_line_v66(data=data,TopologyMP=FinalTopology,model0=FSP_models[c],print_result=False) #Problem: should we check default topology for disptach, or the new one?!
        ...
        for sub in (Bus):
            sub_cont_list = [sub,str(sub+'-1'),str(sub+'-2')]
            # sub_cont_list=[] 
            result = solve_OSP_substation_v66(data=data,cont_list=sub_cont_list,TopologyMP=FinalTopology, model0=OSP_models[sub])
        ...
        OF_OSP_c={}
        for c in line_cont_list: #line_cont_radial:
            result=solve_OSP_substation_v66(data=data,model0=OSP_models[c], cont_list=[c],TopologyMP=FinalTopology)

            # print('\tOSP line cont %3s: %.1f'%(c,result['OF_OSP']))

            OF_OSP_c[c]=result['OF_OSP']
            OF_OSP+=OF_OSP_c[c]          #OF_OSP has values before here
            SP_time += result['time']

            # print('Main loop line_OSP: ',OF_OSP_c[c])
...
# #IEEE_14_bus_Data_PGLib_ACOPF
# #IEEE_14_bus_Data_Congestion
...
# res = BCC_v66_AC_MultiBenders_OptimalSplit(data,line_cont_list=line_cont,Max_Sw_bus=0,
#                                             #    FixedCost=True,Pg_market=Pg_market,Alpha=0.1,
#                                     Probabilistic=True,
#                                    Max_iter=20,Max_FSP_iter=50,FSP_criteria=0,OSP_criteria=0.1,Up_redispatch=0,Dn_redispatch=1)

# # top = res['TopologyDict']
...
# %%
# for c in busbar_cont+coupler_cont+line_cont:
#     print(c,'%.2f '%res['ShedCost_df'].loc[c])
...
# %%
# # # function above with different redispatching

# data=read_data_AC(File='IEEE_14_bus_Data_PGLib_ACOPF.xlsx',DemFactor=1,LineLimit=1,print_data=False)
...
#                         cont_list=busbar_cont+coupler_cont+line_cont
#                         ,Up_redispatch=0,Dn_redispatch=1.0,
#                         Zfixdict=top,
#                         PgFix=res['Pg'],
#                         SolverTime=600,
#                         line_shedding=False,    
#                         Max_Sw_bus=0,print_result=True)



# %%
# for c in busbar_cont+coupler_cont+line_cont:
#     print(c,'%.2f '%res_full['ShedCost_df'].loc[c])
...
# data=read_data(File='IEEE_118_bus_Data.xlsx',print_data=False)

# # line_cont = data['line_cont']
# coupler_cont =data['coupler_cont']
# busbar_cont = data['busbar_cont']


# res_118 = BCC_MultiBenders_Main_v11(data,line_cont_list=[],
#                                    Max_iter=10)
