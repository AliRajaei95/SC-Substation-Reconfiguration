"""Original MIP formulation of security constrained substation reconfiguration
considering busbar, coupler, line contingencies and linear AC-PF equations.
 Original MIP from the Section IV-A case study.

This module also provides the input-data reader and shared utilities used by
the other comparison models.
``SC_SR_OrgMIP`` is also used for the Section
IV-E application on probabilistic security and the Section IV-F market
application on balancing security and cost. These applications are configured
through inputs including ``Probabilistic``, ``FixedCost``, ``Alpha``, and
``Pg_market``.
"""
...
def read_data_AC(
    ...
    """Read a benchmark workbook and prepare the model input dictionary.

    Args:
        File: Path to the Excel workbook containing the network data.
        print_data: Print the prepared network tables when ``True``.
        LineLimit: Multiplier applied to all branch-flow limits.
        DemFactor: Multiplier applied to active and reactive demand.
        remove_line: Line identifiers to remove before building the network.
        line_prob: Probability assigned to each line contingency.
        coupler_prob: Probability assigned to each coupler contingency.
        busbar_prob: Probability assigned to each busbar contingency.

    Returns:
        A dictionary containing network sets, tables, mappings, contingency
        lists and probabilities, graph data, and model constants.
    """
    ...
    # Graph and radial-line identification
    ...
    # Prepared model data
    return data
def Find_radial_lines(
    Lines,
    nodes,
):
    """Identify lines whose removal disconnects the network graph.

    Args:
        Lines: Iterable of directed ``(line, from_bus, to_bus)`` tuples.
        nodes: Iterable of network bus identifiers.

    Returns:
        A list of radial line identifiers.
    """
...
def Build_graph(
    ...
    """Build the undirected NetworkX graph used by topology utilities.
    ...
    Returns:
        An undirected NetworkX graph of the transmission network.
    """
...
# Network-neighborhood utilities
def Find_lines_hops(
    data,
    sub,
    hops=1,
):
    """Find lines within a graph-distance neighborhood of a substation.

    Args:
        data: Prepared network-data dictionary.
        sub: Substation bus at the center of the neighborhood.
        hops: Maximum graph-distance radius.

    Returns:
        A list of line identifiers in the neighborhood.
    """
...
def Find_nodes_hops(
    ...
    """Find buses within a graph-distance neighborhood of a substation.
    ...
    Returns:
        A list of bus identifiers in the neighborhood.
    """
...
def data_cb(
    ...
    """Record incumbent and bound changes during a Gurobi MIP solve.

    Args:
        model: Active Gurobi model with callback state initialized.
        where: Gurobi callback location code.

    Returns:
        None. Progress records are appended to ``model._data``.
    """
...
def solve_economic_dispatch(
    ...
    """Solve the unconstrained economic-dispatch market baseline.

    Args:
        data: Network and model-parameter dictionary returned by
            ``read_data_AC``.
        print_result: Print dispatch and cost results when ``True``.

    Returns:
        A dictionary containing total generation cost and generator dispatch.
    """
    ...
    # Generator limits and power balance
...
def solve_ac_sc_opf(
    ...
    """Solve the linear AC security-constrained market OPF.

    Args:
        data: Network and model-parameter dictionary returned by
            ``read_data_AC``.
        cont_list: Line contingencies enforced in addition to normal operation.
        ...
        delta0_li: Optional branch-angle linearization points; zero values are
            used when omitted.
        print_result: Print dispatch, flows, and cost results when ``True``.

    Returns:
        A dictionary containing the generation cost and active/reactive
        generator dispatch.
    """
    ...
    # Security-constrained market model
    model=gp.Model('AC_SC_OPF')
    model.Params.OutputFlag=0
    ...
    # Power balance and contingency constraints
    ...
    -branch.loc[(l,i,j)]['b_ij']*(theta[i,c]-theta[j,c])
    +Ploss[l,i,j,c]
                            for l,i,j in Lines
    ...
    model.addConstrs( (
        Ploss[l,i,j,c] == Ploss_vol[l,i,j,c] + Ploss_delta[l,i,j,c]
        for l,i,j in Lines for c in cont_list if l!=c
    ), name='eqPloss_total')
    model.addConstrs( (
        Qloss[l,i,j,c] == Qloss_vol[l,i,j,c] + Qloss_delta[l,i,j,c]
        for l,i,j in Lines for c in cont_list if l!=c
    ), name='eqQloss_total')
    model.addConstrs( (
        0 <= Ploss[l,i,j,c] + epsilon[l,i,j,c]
        for l,i,j in Lines for c in cont_list if l!=c
    ), name='eqPloss_pos')

    model.addConstrs( (
        Ploss_vol[l,i,j,c] ==
        branch.loc[(l,i,j)]['g_ij']
        *((Vol0_li[l,i,j,c]-Vol0_li[l,j,i,c])
          /(Vol0_li[l,i,j,c]+Vol0_li[l,j,i,c]))
        *(V2[i,c]-V2[j,c])
        -0.5*branch.loc[(l,i,j)]['g_ij']
        *(Vol0_li[l,i,j,c]-Vol0_li[l,j,i,c])**2
        for l,i,j in Lines for c in cont_list if l!=c
    ), name='eqPloss_vol')
    model.addConstrs( (
        Qloss_vol[l,i,j,c] ==
        -branch.loc[(l,i,j)]['b_ij']
        *((Vol0_li[l,i,j,c]-Vol0_li[l,j,i,c])
          /(Vol0_li[l,i,j,c]+Vol0_li[l,j,i,c]))
        *(V2[i,c]-V2[j,c])
        +0.5*branch.loc[(l,i,j)]['b_ij']
        *(Vol0_li[l,i,j,c]-Vol0_li[l,j,i,c])**2
        for l,i,j in Lines for c in cont_list if l!=c
    ), name='eqQloss_vol')
    ...
    model.addConstrs( (
        Ploss_delta[l,i,j,c] ==
        branch.loc[(l,i,j)]['g_ij']
        *(delta0_li[l,i,j,c]-delta0_li[l,j,i,c])
        *(theta[i,c]-theta[j,c])
        -0.5*branch.loc[(l,i,j)]['g_ij']
        *(delta0_li[l,i,j,c]-delta0_li[l,j,i,c])**2
        for l,i,j in Lines for c in cont_list if l!=c
    ), name='eqPloss_delta')
    model.addConstrs( (
        Qloss_delta[l,i,j,c] ==
...
def SC_SR_OrgMIP(
    ...
    """Solve the original monolithic security-constrained MIP model.

    In addition to the Section IV-A baseline, this function supports the
    Section IV-E probabilistic-security application and the Section IV-F
    market application for balancing security and cost. Set ``Probabilistic``
    to weight contingency costs by their probabilities. Set ``FixedCost`` and
    provide ``Pg_market`` and ``Alpha`` to constrain operating cost relative
    to the reference market dispatch.
    ...
    Returns:
        A dictionary containing topology, dispatch, costs, load shedding,
        solver statistics, convergence data, and solve time.
    """
    ...
    -branch.loc[(l,i,j)]['b_ij']*(delta_li[l,i,j,c]-delta_li[l,j,i,c])
    +Ploss[l,i,j,c]
                            for l,i,j in Lines
                           for c in cont_list if l!=c   ) , name='eqPij')
    ...
    model.addConstrs( (
        ...
        branch.loc[(l,i,j)]['g_ij']
        *(delta0_li[l,i,j,c]-delta0_li[l,j,i,c])
        *(delta_li[l,i,j,c]-delta_li[l,j,i,c])
        -0.5*branch.loc[(l,i,j)]['g_ij']
        *(delta0_li[l,i,j,c]-delta0_li[l,j,i,c])**2
        for l,i,j in Lines for c in cont_list if l!=c
    ), name='eqPloss_delta')
    ...
    # Update the branch-flow linearization points from the final solution.
    for l,i,j in Lines:
        for c in cont_list:
            Vol0_li[l,i,j,c] = np.sqrt(V2_li[l,i,j,c].x)
            delta0_li[l,i,j,c] = delta_li[l,i,j,c].x
